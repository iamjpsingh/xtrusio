...
**Local services architecture (revised after starting execution):**
- **Supabase stack** (Postgres 17 + pgvector, GoTrue, PostgREST, Realtime, Studio, Inbucket) is managed by the **Supabase CLI** (`supabase start`). The CLI spawns its own Docker containers on its own network. We do **not** put Supabase services in our `docker-compose.yml`.
- **Our `docker-compose.yml`** owns only **Valkey** (Supabase doesn't ship a cache).
- **Container conventions for the services we own** (currently only Valkey):
  - Named `xtrusio-` prefix → `xtrusio-valkey`.
  - Run on `xtrusio-net` (Docker bridge network).
  - Non-default host ports to avoid collisions: Valkey on `63792`.
- **Supabase host ports** are CLI defaults: API gateway `54321`, Postgres `54322`, Studio `54323`, Inbucket `54324`. Apps connect to Supabase via these host ports.
...
- `docker-compose.yml` — Valkey on `xtrusio-net` (Postgres + Supabase services are managed by Supabase CLI, not this file)
- `supabase/config.toml` — Supabase CLI config (created by `supabase init`)
- `supabase/migrations/00000000000000_init_extensions.sql` — first migration (vector, pgcrypto, citext)
...
## Task 12: Initialize Supabase CLI + Valkey via Docker Compose

> **Revised during execution (2026-05-08):** Original plan put Postgres in our docker-compose.yml. Switched to Supabase CLI per user requirement. Supabase CLI manages Postgres + GoTrue + PostgREST + Realtime + Studio. Our docker-compose.yml owns only Valkey. The plan has been updated to match what was actually executed; the rejected steps (init.sql under infra/postgres, postgres service in compose) are NOT part of this task anymore.

**Prerequisites:** Supabase CLI installed (`brew install supabase/tap/supabase-beta` on macOS).

**Files:**
- Create: `supabase/config.toml` (auto-generated by `supabase init`)
- Create: `supabase/migrations/00000000000000_init_extensions.sql` (manual)
- Create: `docker-compose.yml` (Valkey only)

- [ ] **Step 0a: Install Supabase CLI**

```bash
brew install supabase/tap/supabase-beta
supabase --version
# Expected: prints e.g. "2.99.0-beta.2"
```

- [ ] **Step 0b: Initialize Supabase config**

```bash
supabase init
```

Expected output: `Finished supabase init.` Creates `supabase/config.toml` and `supabase/.gitignore` (which already ignores `.temp/` and `.env.local`).

The `config.toml` defaults are sensible:
- `project_id = "xtrusio"`
- API on port 54321, DB on port 54322, Studio on port 54323
- Postgres `major_version = 17`

- [ ] **Step 1: Add the first SQL migration to enable extensions**

Create `supabase/migrations/00000000000000_init_extensions.sql`:
...
- [ ] **Step 1.5: Original `infra/postgres/init.sql` plan is OBSOLETE — skip and ensure that path does not exist** (handled automatically; no `infra/postgres/` directory in the final layout).
...
- [ ] **Step 2: Create `docker-compose.yml`** (Valkey only — Supabase managed by CLI)

```yaml
name: xtrusio

# Note: Postgres + GoTrue + Realtime + PostgREST + Studio are managed by the
# Supabase CLI (`supabase start`), which spawns its own Docker containers on
# its own internal network. This file owns ONLY services that Supabase CLI
# does not provide — currently just Valkey for caches/queues.
...
> **Why this shape:**
> - `name: xtrusio` pins the Compose project name.
> - `container_name: xtrusio-valkey` pins the container name (otherwise Compose adds `-1`).
> - `networks.xtrusio-net.name: xtrusio-net` pins the network name (otherwise Compose prefixes with project name).
> - Host port `63792` is deliberately uncommon to avoid collisions. Future containers we own that need cache access join `xtrusio-net` and resolve `valkey:6379` by name.
> - Supabase services live on the Supabase-CLI-managed network, separate from `xtrusio-net`. Our apps reach them via host ports (54321/54322/etc).
...
Expected: Supabase CLI pulls images on first run (1-3 minutes), then prints a status block with API URL, DB URL, Studio URL, and the anon + service-role keys. Copy the keys into your `.env` if you have one.

- [ ] **Step 4: Verify the migration ran (extensions installed)**

```bash
psql 'postgresql://postgres:postgres@localhost:54322/postgres' -c '\dx'
```

Expected output includes: `vector`, `pgcrypto`, `citext`. (psql comes with Supabase CLI's bundled tools, or use `brew install libpq` if not present.)

- [ ] **Step 5: Bring up Valkey**
...
Expected output: `xtrusio-valkey` (Supabase containers are on Supabase's own network, not `xtrusio-net`).
...
```bash
git add supabase/ docker-compose.yml
git commit -m "feat(infra): Supabase CLI + named xtrusio-valkey container on xtrusio-net"
```
...
3. `make db-up` brings up Supabase (via CLI) and Valkey, and prints the URL block.
4. `make dev` brings up DB stack + API on `:8000` + web on `:5173`.
5. `curl http://localhost:8000/health` returns `{"status":"ok"}`.
6. `curl http://localhost:5173` returns HTTP 200.
7. `psql 'postgresql://postgres:postgres@localhost:54322/postgres' -c '\dx'` shows `vector`, `pgcrypto`, `citext`.
8. `curl http://localhost:54321/auth/v1/health` returns 200 (GoTrue health).
9. `docker exec xtrusio-valkey valkey-cli ping` returns `PONG`.
10. `docker network inspect xtrusio-net` shows `xtrusio-valkey` attached.
11. `docker port xtrusio-valkey 6379` returns `0.0.0.0:63792`.
12. `apps/api/src/xtrusio_api/main.py` and `apps/web/src/App.tsx` exist with the exact contents shown in Tasks 7 and 9.
13. `mise.toml` exists pinning Node 22, Python 3.12, pnpm 10.
14. `.pre-commit-config.yaml` exists; `uv run pre-commit run --all-files` passes.
15. `supabase/config.toml` exists; first migration applied cleanly.
16. The repo has zero `.github/workflows/*` files and no CI/CD setup, per project policy.
