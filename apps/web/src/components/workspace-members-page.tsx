          ...
          <ul className="divide-y rounded-md border">
            {invites.items.map((i: TenantInvite) => (
              <li key={i.id} className="flex items-center justify-between p-4">
                ...
                {i.accepted_at ? (
                  // ACCEPTED → "Clear" removes the now-redundant invite record
                  // from the list (the invitee is already a member). Same DELETE
                  // endpoint as Revoke; the backend returns 204 either way.
